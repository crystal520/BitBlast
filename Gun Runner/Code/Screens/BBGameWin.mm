...
- (id) init {
    if(([super init])) {
        ...
        gameWinLabel = [CCLabelBMFont labelWithString:@"" fntFile:@"medium.fnt" width:winSize.width * 0.95 alignment:kCCTextAlignmentCenter];
...
- (void)onEnter {
	[[[CCDirector sharedDirector] touchDispatcher] addTargetedDelegate:self priority:TOUCH_DEPTH_MENU swallowsTouches:YES];
	[super onEnter];
}

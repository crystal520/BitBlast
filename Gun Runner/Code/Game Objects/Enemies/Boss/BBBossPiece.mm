...
- (id) initWithDictionary:(NSDictionary *)initDict {
    if((self = [super init])) {
        ...
        dummyPosition = ccpAdd(ccpMult(CGPointFromString([initDict objectForKey:@"position"]), [ResolutionManager sharedSingleton].positionScale), ccp([self displayFrame].rect.size.width * self.anchorPoint.x, [self displayFrame].rect.size.height * self.anchorPoint.y));

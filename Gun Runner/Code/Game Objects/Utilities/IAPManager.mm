...
- (void) requestIAP {
	...
	[request release];
	// set timeout
	[[[CCDirector sharedDirector] scheduler] scheduleSelector:@selector(requestTimeout) forTarget:self interval:30 paused:NO];
}

- (void) requestTimeout {
	// make sure this function doesn't continue to get called
	[[[CCDirector sharedDirector] scheduler] unscheduleSelector:@selector(requestTimeout) forTarget:self];
